...
const {handleGetRoot,handleGetUsers,handleGetUserId,handleGetUsersName,handleSortInId} = require('./serverFunctions/getFunc.js')
const handlePostNewUser = require('./serverFunctions/postFunc.js')
const handleUpdateUserName = require('./serverFunctions/put_patchFunc')
const handleDeletUser = require('./serverFunctions/deletFunc.js')
const port = 3001;
...
const server = http.createServer((req,res)=>{
  switch(true){
    ...
    break
    case req.url.match(/\/api\/users\/([0-9]+)/) && req.method === 'PATCH':
    handleUpdateUserName(req,res)
    break  
    case req.url.match(/\/api\/users\/([0-9]+)/) && req.method === 'DELETE':
    handleDeletUser(req,res)
    break  
    case req.url === '/api/users/sort' && req.method === 'GET':
      handleSortInId(req,res)  
    break  
  }
})
